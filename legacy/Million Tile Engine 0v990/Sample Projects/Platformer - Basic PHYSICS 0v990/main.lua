...
mte.update()
mte.physics.addBody(player, "dynamic", {friction = 0.2, bounce = 0.0, density = 1, filter = { categoryBits = 1, maskBits = 1 } })
...
local jump = function(event)
	if event.phase == "began" then
		display.getCurrentStage():setFocus(event.target, event.id)
		event.target.isFocus = true
		player:applyLinearImpulse(0, -70, player.x, player.y)
	end
	if event.phase == "ended" or event.phase == "cancelled" then
		display.getCurrentStage():setFocus( event.target, nil )
		event.target.isFocus = false
...
local gameLoop = function(event)
	mte.debug()		
	mte.update()
	player:applyForce(acc * 500, 0, player.x, player.y)
end
