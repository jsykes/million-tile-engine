...
local sliceWidth = display.contentWidth / slices
local sliceSeq = {}
local sliceFrame = {}
...
local halfContentHeight = contentHeight / 2
...
local setupView = function(tfov, tslices)
	blockScaleZ = blockScaleX
	fov = tfov
	slices = tslices
	sliceSize = fov/slices
	sliceWidth = display.contentWidth / slices
	--distToProjection = (slices * 0.5) / math.tan(math.rad(fov * 0.5))
	distToProjection = (display.contentWidth * 0.5) / math.tan(math.rad(fov * 0.5))
	toWidth = math.tan(math.rad(sliceSize * 0.5)) * 2
	widthMod = map.tilesets[1].tilewidth / blockScaleX
end
...
local createView = function()
	...
	for i = 1, slices, 1 do
		local left = 0 + sliceWidth * (i - 1) + (sliceWidth * 0.5)
		local top = display.contentHeight / 2
		local sliceHeight = display.contentHeight
		view[i] = display.newSprite(sliceGroup, wallSheet[1].sheet, sequenceData)
		view[i].x = left
		view[i].y = top
		view[i].width = sliceWidth
		view[i].height = sliceHeight
		view[i]:setSequence(1)
		sliceSeq[i] = 0
		sliceFrame[i] = 0
	end
...
local projection = function(direction, directionY)
	...
	for i = 1, slices, 1 do
		...
		--angleR = math.rad(angleD)
		local angleR = angleD * toRadian
		
		local slope = math.tan(angleR)	
		--local slope = angleR * toRadian
		...
		if x > 0 and y < 0 then
			...
			local angle = math.rad(angleD - 270)
			local checkY = tileY
			local checkX = math.tan(angle) * startY + cameraX
			...
			if locX <= worldSizeX and locX > 0 and locY <= worldSizeY and locY > 0 then
				if map.layers[refLayer].world[locX][locY] ~= 0 then
					
					wallX = {locX, locY, checkX, checkY}
				end
			end
			...
			local angle = math.rad(360 - angleD)
			local checkY = cameraY - (math.tan(angle) * (tileX + blockScaleX - cameraX))
		...
		if x < 0 and y > 0 then
			...
			--check horizontal walls
			local angle = math.rad(angleD - 90)
			local checkY = tileY + blockScaleY + 1
			...
			--check vertical walls
			local angle = math.rad(180 - angleD)
		...
		if x < 0 and y < 0 then
			...
			--check horizontal walls
			local angle = math.rad(270 - angleD)
			local checkY = tileY
			...
			--check vertical walls
			local angle = math.rad(angleD - 180)
		...
		if wallX and wallY then
			view[i].isVisible = true
			--local distanceH = math.sqrt( math.pow((wallX[3] - cameraX), 2) + math.pow((wallX[4] - cameraY), 2) )
			local distanceH = math.sqrt( ((wallX[3] - cameraX) * (wallX[3] - cameraX)) + ((wallX[4] - cameraY) * (wallX[4] - cameraY)) )
			--local distanceV = math.sqrt( math.pow((wallY[3] - cameraX), 2) + math.pow((wallY[4] - cameraY), 2) )
			local distanceV = math.sqrt( ((wallY[3] - cameraX) * (wallY[3] - cameraX)) + ((wallY[4] - cameraY) * (wallY[4] - cameraY)) )
		...
		elseif wallX then
			view[i].isVisible = true
			--distance = math.sqrt( math.pow((wallX[3] - cameraX), 2) + math.pow((wallX[4] - cameraY), 2) ) 
			distance = math.sqrt( ((wallX[3] - cameraX) * (wallX[3] - cameraX)) + ((wallX[4] - cameraY) * (wallX[4] - cameraY)) )
			wall = wallX
			toggle = 1
		elseif wallY then
			view[i].isVisible = true
			--distance = math.sqrt( math.pow((wallY[3] - cameraX), 2) + math.pow((wallY[4] - cameraY), 2) )
			distance = math.sqrt( ((wallY[3] - cameraX) * (wallY[3] - cameraX)) + ((wallY[4] - cameraY) * (wallY[4] - cameraY)) )
			wall = wallY
			toggle = 2
		else
			view[i].isVisible = false
		end
		
		if distance then
			local temp = sliceSize * (i - (slices / 2))
			--local dist = distance * math.cos(math.rad(temp))
			local dist = distance * math.cos(temp * toRadian)
			local distA = distance
			distance = dist

			--local sliceHeight = (map.tilesets[1].tilewidth / distance) * distToProjection
			local sliceHeight = (blockScaleX / distance) * distToProjection

			local brightness = (150 * (sliceHeight / display.contentHeight * 0.85) + (255 * 0.15)) + 105
			...
			if x > 0 and y < 0 then
				if toggle == 1 then
					offset = math.fmod(wall[3], blockScaleX)
				elseif toggle == 2 then
					offset = math.fmod(wall[4], blockScaleY)
				end
			end
			if x > 0 and y > 0 then
				if toggle == 1 then
					offset = math.fmod((worldSizeX * blockScaleX) - wall[3], blockScaleX)
				elseif toggle == 2 then
					offset = math.fmod(wall[4], blockScaleY)
				end
			end
			if x < 0 and y > 0 then
				if toggle == 1 then
					offset = math.fmod((worldSizeX * blockScaleX) - wall[3], blockScaleX)
				elseif toggle == 2 then
					offset = math.fmod((worldSizeY * blockScaleY) - wall[4], blockScaleY)
				end
			end
			if x < 0 and y < 0 then
				if toggle == 1 then
			...
			local index = math.ceil(blockScaleX / width)
			if index > blockScaleX then
				index = blockScaleX
			end
			
			--local tileID = map.layers[1].world[wall[1]][wall[2]]
			--local indexB = wallSheet[tileID].bookmarks[index]
			--local frameWidth = map.tilesets[1].tilewidth / index			
			--local index2 = math.ceil(offset / width)
			--local index3 = indexB
			--local index4 = index3 + index2 - 1
			--local index4 = math.round(index4)
			
			local tileID = map.layers[1].world[wall[1]][wall[2]]
			local frameWidth = tileWidth / index			
			local index2 = math.ceil(offset / width)
			local index3 = wallSheet[tileID].bookmarks[index]
			local index4 = math.round(index3 + index2 - 1)
			
			--DRAW WALL SEGMENT
			if tileID ~= sliceSeq[i] then
				view[i]:setSequence(tileID)
				sliceSeq[i] = tileID
			end
			if index4 ~= sliceFrame[i] then
				view[i]:setFrame(index4)
				sliceFrame[i] = index4
			end
			view[i].height = sliceHeight
			view[i].width = sliceWidth
			view[i]:setFillColor(brightness, brightness, brightness)
