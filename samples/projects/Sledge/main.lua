...
local map = mte.loadMap("Sledge/map/sledge.tmx")
...
local shadow = display.newImageRect("Sledge/shadow.png", 75, 75)
shadow.alpha = 0.5
...
local player = display.newImageRect("Sledge/player.png", 32, 64)
local setup = {layer = 1, kind = "imageRect", locX = locX, locY = locY, 
...
--SETUP CONTROLS ----------------------------------------------------------------------
local Dpad = display.newImageRect("Sledge/Dpad.png", 120, 120)
...
Gas.alpha = 0.6
Gas.x = Dpad.x + Dpad.width + 10
--jumpBtn.x = Screen.Right - jumpBtn.width*0.5 - 10
Gas.y = Screen.Bottom - Gas.height*0.5 - 10
...
local gameLoop = function(event)
    ...
    mapObj.xScale = MAP_SCALE / (player.offsetZ + 1)
    mapObj.yScale = MAP_SCALE / (player.offsetZ + 1)
